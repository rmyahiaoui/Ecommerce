...
class RawFileUpload
  ...
  def convert_and_call(env)
    ...
    env['rack.input'].each do |chunk|
      # ruby 1.9 only tempfile << chunk.force_encoding('UTF-8')
      tempfile << chunk
    end
    tempfile.flush
    tempfile.rewind
    ...
    fake_file = {
      :uploaded_data => {
        ...
        :content_type => MIME::Types.type_for(env['HTTP_X_FILE_NAME']).first,
    ...
    env['rack.request.form_hash']['my_attachment'] = fake_file
    env['rack.request.query_hash']['my_attachment'] = fake_file
    @app.call(env)
  end
end